...
function loadChat(chatId) {
  ...
  chatArea.innerHTML = `<div class="subject-banner">📘 ${escapeHTML(selectedSubject)} Mode Activated</div>`;

  chatData.forEach(msg => {
    chatArea.innerHTML += `
      <div class="user-msg"><div class="msg-text">${escapeHTML(msg.q)}</div></div>
...
async function askAI() {
  ...
  /* PRIORITY 2: Local Subject Fallback */
  ...
  try {
    ...
    displayAnswer(
      chatArea,
      question,
      formatAnswer(data.answer || "⚠️ No answer received."),
      loading
    );
    ...
    const subjectKey =
      SUBJECT_MAP[selectedSubject.toLowerCase()] || selectedSubject.toLowerCase();

    const tip =
      offlineTips[subjectKey] ||
      `💡 Please ask a specific question about ${selectedSubject}.`;
...
async function uploadImage() {
  ...
  chat.innerHTML += `
    <div class="user-msg">
      <div class="msg-text">
        ...
        <small style="opacity:0.85;">❓ ${escapeHTML(question)}</small>
      </div>
    </div>
  `;
  ...
  try {
    ...
    const res = await fetch("/ask-from-image", {
      method: "POST",
      body: formData
    });
    ...
    if (data.extracted_text && data.extracted_text.trim() !== "") {
      ...
      chat.innerHTML += `
        <div class="ai-msg" style="font-size:0.82em;opacity:0.7;border-left:3px solid #aaa;padding-left:8px;margin-bottom:4px;">
          📄 <b>Extracted Text:</b><br>${escapeHTML(preview)}
        </div>
      `;
    } else {
      chat.innerHTML += `
    ...
    if (error.message.includes("Failed to fetch")) {
      userMsg = "Cannot connect to server. Make sure your backend is running.";
    }

    chat.innerHTML += `<div class="ai-msg" style="color:#e74c3c;">⚠️ Error: ${escapeHTML(userMsg)}</div>`;
...
async function checkGrammar() {
  ...
  if (text === "") {
    resultBox.innerText = "⚠️ Please enter a sentence";
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error("Grammar server error: " + res.status);
    }

    const data = await res.json();
    resultBox.innerHTML = `✅ Corrected: <b>${escapeHTML(data.corrected_text || text)}</b>`;

  } catch (error) {
    console.warn("Grammar backend offline:", error.message);

    let input = text.toLowerCase().trim();
    let corrected = text;

    if (typeof fallback_grammar !== "undefined" && Array.isArray(fallback_grammar)) {
    ...
    resultBox.innerHTML = `✅ Corrected: <b>${escapeHTML(corrected)}</b>`;
  }
}
